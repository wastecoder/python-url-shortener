...
- [ ] **Validação de destino com resolução de nome.** A `url_policy` da Fase 1 decide só pela string: ela recusa endereço literal não roteável, `localhost`, os sufixos internos, host de rótulo único e as formas numéricas que o `ipaddress` não lê mas o navegador resolve. O que ela não faz é resolver `evil.com` e descobrir que aponta para `127.0.0.1`. Fechar isso é sair do domínio puro — resolução é I/O — então a validação viraria uma porta (`HostResolver`) com implementação no adaptador, e o teste unitário continuaria rodando contra um fake
- [ ] Duas armadilhas do item acima, e é por elas que ele não entrou no V1: resolver **não fecha** o DNS rebinding, porque o nome pode responder um endereço na checagem e outro no clique — só uma verificação no momento da requisição fecha; e uma resolução na criação do link põe uma chamada de rede no caminho de escrita, com timeout, retry e um novo modo de falha
